...
from data_processors import InputFeatures, PaddingInputExample, \
    QcFineProcessor, GeneralProcessor

flags = tf.flags

FLAGS = flags.FLAGS

flags.DEFINE_integer('unlabeled_multiplier', 100,
...
flags.DEFINE_bool('do_test', False,
                  'Whether to run test on the dev set.')

flags.DEFINE_bool('do_predict', False,
                  'Whether to run the model in inference mode on the test set.'
                  )

flags.DEFINE_bool('do_val', False,
                  'Whether to run validation on the dev set.')

flags.DEFINE_bool('do_eval_predict', False,
                  'Whether to run the model in inference mode on the validation set.'
                  )

flags.DEFINE_bool('pred_OOS', False,
                  'Whether to Test on OOS data or not')
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(all_examples):
        ...
        features['label_mask'] = \
            create_int_feature([feature.label_mask])
...
def file_based_input_fn_builder(
    ...
    # For eval, we want no shuffling and parallel reading doesn't matter.
        ...
        d = d.apply(tf.contrib.data.map_and_batch(lambda record: \
                    _decode_record(record, name_to_features),
...
def create_model(
    ...
    (D_real_features, D_real_logits, D_real_prob) = discriminator(
        output_layer,
        hidden_size,
        keep_prob,
        is_training,
    ...
    if is_training:
        ...
        labeled_example_count = tf.cast(tf.size(per_example_loss),
                tf.float32)
    ...
    z = tf.random_uniform(
        [FLAGS.train_batch_size, LATENT_Z],
        minval=0,
        maxval=1,
        dtype=tf.float32,
        seed=SEED,
    ...
    x_g = generator(z, hidden_size, keep_prob, is_training=is_training,
                    reuse=False)
    tf.print(x_g,[x_g])


    (D_fake_features, DU_fake_logits, DU_fake_prob) = discriminator(
        x_g,
        hidden_size,
        keep_prob,
        is_training,
    ...
    D_L_unsupervised1U = -1 * tf.reduce_mean(tf.math.log(1
            - D_real_prob[:, 0] + epsilon))

    D_L_unsupervised2U = -1 * tf.reduce_mean(tf.math.log(DU_fake_prob[:
            , 0] + epsilon))
    d_loss = D_L_Supervised + D_L_unsupervised1U + D_L_unsupervised2U

    g_loss = -1 * tf.reduce_mean(tf.math.log(1 - DU_fake_prob[:, 0]
                                 + epsilon))

    G_feat_match = \
        tf.reduce_mean(tf.square(tf.reduce_mean(D_real_features,
                       axis=0) - tf.reduce_mean(D_fake_features,
                       axis=0)))
...
def model_fn_builder(
    ...
    # for name in sorted(features.keys()):
      # tf.print("  name = %s, shape = %s", name, features[name].shape)
        ...
        (d_loss, g_loss, per_example_loss, logits, probabilities) = \
            create_model(
            bert_config,
            is_training,
            input_ids,
        ...
        d_vars = bert_vars + [v for v in tvars if 'Discriminator'
                              in v.name]
      ...
      # tf.print("  name = %s, shape = %s%s", var.name, var.shape,init_string)

        output_spec = None
        if mode == tf.estimator.ModeKeys.TRAIN:

            d_train_op = optimization.create_optimizer(
                'd',
                d_vars,
                d_loss,
                learning_rate,
            ...
            g_train_op = optimization.create_optimizer(
                'g',
                g_vars,
                g_loss,
                learning_rate,
            ...
            logging_hook = \
                tf.train.LoggingTensorHook({'d_loss': d_loss,
                    'g_loss': g_loss,
                    'per_example_loss': per_example_loss},
                    every_n_iter=1)

            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode,
                    loss=d_loss + g_loss, train_op=tf.group(d_train_op,
                    g_train_op), training_hooks=[logging_hook],
            ...
            def metric_fn(
                ...
                precision = tf_metrics.precision(labels=label_ids,
                        predictions=predictions,
                        num_classes=num_labels, weights=is_real_example)
            ...
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode,
                    loss=d_loss, eval_metrics=eval_metrics,
                    scaffold_fn=scaffold_fn)
        else:
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode,
...
def evaluate(
    ...
    eval_file = os.path.join(FLAGS.output_dir, 'eval_' + str(task_name)
                             + '.tf_record')
    ...
    tf.print('  Num examples = %d (%d actual, %d padding)',
             len(eval_examples), num_actual_eval_examples,
             len(eval_examples) - num_actual_eval_examples)
    tf.print('  Batch size = %d', FLAGS.eval_batch_size)
    ...
    overall_result_file = open(task_name + '_statistics_GANBERT'
                               + str(label_rate) + '.txt', 'a+')
    ...
    if is_OOS:
        output_eval_file = os.path.join(FLAGS.output_dir,
                'OOS_eval_results_' + str(task_name) + '.txt')
    else:
        output_eval_file = os.path.join(FLAGS.output_dir,
    ...
    with tf.gfile.GFile(output_eval_file, 'w') as writer:
        print('***** Eval results *****')
        for key in sorted(result.keys()):
            tf.print('  %s = %s', key, str(result[key]))
            writer.write('%s = %s\n' % (key, str(result[key])))
...
def main(_):
    ...
    if not FLAGS.do_train and not FLAGS.do_test and not FLAGS.do_val \
        and not FLAGS.do_predict and not FLAGS.do_eval_predict \
        and not FLAGS.pred_OOS:
        raise ValueError("At least one of `do_train`, `do_test` or `do_val' or` do_predict' or `do_eval_predict' or 'pred_OOS' must be True."
                         )
    ...
    tokenizer = \
        tokenization.FullTokenizer(vocab_file='cased_L-12_H-768_A-12/vocab.txt'
                                   , do_lower_case=FLAGS.do_lower_case)

    tpu_cluster_resolver = None
    if FLAGS.use_tpu and FLAGS.tpu_name:
        tpu_cluster_resolver = \
            tf.contrib.cluster_resolver.TPUClusterResolver(FLAGS.tpu_name,
                zone=FLAGS.tpu_zone, project=FLAGS.gcp_project)

    is_per_host = tf.contrib.tpu.InputPipelineConfig.PER_HOST_V2

    run_config = tf.contrib.tpu.RunConfig(cluster=tpu_cluster_resolver,
            master=FLAGS.master, model_dir=FLAGS.output_dir,
    ...
    if FLAGS.do_train:
        labeled_examples = \
            processor.get_labeled_examples(FLAGS.data_dir)
        unlabeled_examples = \
            processor.get_unlabeled_examples(FLAGS.data_dir)

        num_train_examples = len(labeled_examples) \
  ...
  # or GPU.
    ...
    if FLAGS.do_train:
        ...
        real_num_train_steps = int(num_written_examples
                                   / FLAGS.train_batch_size
                                   * FLAGS.num_train_epochs)

        print('***** Running training *****')

        # tf.print("  Num examples = %d", len(labeled_examples) + len(unlabeled_examples))
        # tf.print("  Batch size = %d", FLAGS.train_batch_size)
        # tf.print("  Num steps = %d", real_num_train_steps)

        train_input_fn = \
            file_based_input_fn_builder(input_file=train_file,
                seq_length=FLAGS.max_seq_length, is_training=True,
                drop_remainder=True)
        estimator.train(input_fn=train_input_fn,
                        max_steps=real_num_train_steps)
    ...
    if FLAGS.do_val:
        ...
        eval_examples = \
            processor.get_validation_examples(FLAGS.data_dir)
        evaluate(
            estimator=estimator,
            label_rate=label_rate,
            eval_examples=eval_examples,
    ...
    if FLAGS.pred_OOS:
        ...
        oos_eval_examples = \
            processor.get_OOS_test_examples(FLAGS.data_dir)
        evaluate(
            estimator=estimator,
            label_rate=label_rate,
            eval_examples=oos_eval_examples,
            task_name=task_name,
            label_list=label_list,
            tokenizer=tokenizer,
            is_OOS=True,
    ...
    if FLAGS.do_predict:
        print('===================== Inference model on Test Data ======================'
              )
    ...
    # tf.print("  Num examples = %d (%d actual, %d padding)",
                    ...
                    # len(predict_examples) - num_actual_predict_examples)
    # tf.print("  Batch size = %d", FLAGS.predict_batch_size)

        predict_drop_remainder = (True if FLAGS.use_tpu else False)
        predict_input_fn = \
            file_based_input_fn_builder(input_file=predict_file,
                seq_length=FLAGS.max_seq_length, is_training=False,
    ...
    if FLAGS.do_eval_predict:
        print('===================== Inference model on Validation Data ====================='
              )
        predict_examples = \
            processor.get_validation_examples(FLAGS.data_dir)
        num_actual_predict_examples = len(predict_examples)
