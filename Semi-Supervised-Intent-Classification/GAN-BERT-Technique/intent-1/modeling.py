  ...
  """
    ...
    """
        ...
        # for the attention scores.
                ...
                attention_mask = \
                    create_attention_mask_from_input_mask(input_ids,
                        input_mask)
      ...
      # dimensional representation of the segment.
        ...
        # to the first token. We assume that this has been pre-trained
                ...
                first_token_tensor = tf.squeeze(self.sequence_output[:,
                        0:1, :], axis=1)
                self.pooled_output = \
                    tf.layers.dense(first_token_tensor,
                                    config.hidden_size,
                                    activation=tf.tanh,
                                    kernel_initializer=create_initializer(config.initializer_range))
                print('**************Pooled Output of BERT Model***************')
    ...
    def get_embedding_table(self):
        print('***************Embedding Table of BERT Model***************')
...
def gelu(x):
  ...
  """
    ...
    cdf = 0.5 * (1.0 + tf.tanh(np.sqrt(2 / np.pi) * (x + 0.044715
                 * tf.pow(x, 3))))
    return x * cdf
...
def layer_norm(input_tensor, name=None):
    ...
    return tf.contrib.layers.layer_norm(inputs=input_tensor,
            begin_norm_axis=-1, begin_params_axis=-1, scope=name)
...
def embedding_lookup(
  ...
  # reshape to [batch_size, seq_length, 1].
    ...
    output = tf.reshape(output, input_shape[0:-1] + [input_shape[-1]
                        * embedding_size])
...
def create_attention_mask_from_input_mask(from_tensor, to_mask):
  ...
  """
    ...
    to_mask = tf.cast(tf.reshape(to_mask, [batch_size, 1,
                      to_seq_length]), tf.float32)
...
def attention_layer(
  ...
  """
    ...
    def transpose_for_scores(
        ...
        output_tensor = tf.reshape(input_tensor, [batch_size,
                                   seq_length, num_attention_heads,
                                   width])
  ...
  # `query_layer` = [B*F, N*H]
    ...
    query_layer = tf.layers.dense(from_tensor_2d, num_attention_heads
                                  * size_per_head,
  ...
  # `key_layer` = [B*T, N*H]
    ...
    key_layer = tf.layers.dense(to_tensor_2d, num_attention_heads
                                * size_per_head, activation=key_act,
                                name='key',
                                kernel_initializer=create_initializer(initializer_range))

  # `value_layer` = [B*T, N*H]

    value_layer = tf.layers.dense(to_tensor_2d, num_attention_heads
                                  * size_per_head,
  ...
  # `value_layer` = [B, T, N, H]
    ...
    value_layer = tf.reshape(value_layer, [batch_size, to_seq_length,
                             num_attention_heads, size_per_head])
  ...
  # `context_layer` = [B, F, N, H]
    ...
    # `context_layer` = [B*F, N*H]
        ...
        context_layer = tf.reshape(context_layer, [batch_size
                                   * from_seq_length,
                                   num_attention_heads * size_per_head])
    else:
    ...
    # `context_layer` = [B, F, N*H]
        ...
        context_layer = tf.reshape(context_layer, [batch_size,
                                   from_seq_length, num_attention_heads
                                   * size_per_head])
  ...
  # help the optimizer.
    ...
    for layer_idx in range(num_hidden_layers):
        ...
        # with `layer_input`.
                ...
                with tf.variable_scope('output'):
                    attention_output = \
                        tf.layers.dense(attention_output, hidden_size,
                            kernel_initializer=create_initializer(initializer_range))
    ...
    if do_return_all_layers:
        ...
        for layer_output in all_layer_outputs:
            final_output = reshape_from_matrix(layer_output,
                    input_shape)
