...
class QcFineProcessor(DataProcessor):
    ...
    def get_labeled_examples(self, data_dir):
        ...
        return self._create_examples(os.path.join(data_dir,
                'labeled_Q1.tsv'), 'train')

    def get_unlabeled_examples(self, data_dir):
        """See base class."""

        return self._create_examples(os.path.join(data_dir,
                'unlabeled_Q1.tsv'), 'train')

    def get_test_examples(self, data_dir):
        """See base class."""

        return self._create_examples(os.path.join(data_dir, 'test_Q1.tsv'
                ), 'test')

    def get_labels(self, data_dir, set_type):
        """See base class."""

        return ['UNK_UNK', 'Prov_Info', 'Ask_info']
...
class GeneralProcessor(DataProcessor):
    ...
    def get_labels(self, data_dir, set_type='gan_bert'):
        ...
        (_, unique_label_list) = \
            self._create_examples(os.path.join(data_dir, 'train_Q1.tsv'),
                                  'train')
