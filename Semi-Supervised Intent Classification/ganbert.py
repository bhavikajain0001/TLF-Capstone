...
from data_processors import InputFeatures, PaddingInputExample, QcFineProcessor, GeneralProcessor
...
def file_based_convert_examples_to_features(
    labeled_examples, unlabeled_examples, label_list, max_seq_length, tokenizer, output_file, label_mask_rate,type_):
  ...
  def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        # For each epoch...
        training_stats = []
        for epoch_i in range(0, int(FLAGS.num_train_epochs)):
          # ========================================
          #               Training
          # ========================================
          # Perform one full pass over the training set.
          print("")
          print('======== Epoch {:} / {:} ========'.format(epoch_i + 1, int(FLAGS.num_train_epochs)))
          print('Training...')
...
def evaluate(estimator, label_rate, eval_examples, task_name, label_list, tokenizer,is_OOS=True):
    ...
    file_based_convert_examples_to_features(
        eval_examples, None, label_list, FLAGS.max_seq_length, tokenizer, eval_file, label_mask_rate=1, type_ = 'eval')
    ...
    print('seq_length: ', FLAGS.max_seq_length)
...
def main(_):
  ...
  processors = {"qc-fine": QcFineProcessor, "general": GeneralProcessor}

  tokenization.validate_case_matches_checkpoint(FLAGS.do_lower_case,
                                                FLAGS.init_checkpoint)

  if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.pred_OOS:
    raise ValueError("At least one of `do_train`, `do_eval` or `do_predict' or 'pred_OOS' must be True.")
  ...
  label_list = processor.get_labels(FLAGS.data_dir, set_type='gan_bert')
  ...
  if FLAGS.do_train:
    ...
    num_written_examples = file_based_convert_examples_to_features(
        ...
        label_mask_rate=label_rate, type_='train')
  ...
  if FLAGS.do_eval:
    ...
    if FLAGS.pred_OOS:       
        ...
        evaluate(estimator=estimator, label_rate=label_rate, eval_examples=oos_eval_examples,
                  task_name=task_name, label_list=label_list, tokenizer=tokenizer,is_OOS=True)
